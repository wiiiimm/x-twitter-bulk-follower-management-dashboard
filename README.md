...
6. **Unfollow** or **Unfollow all (selected)** adds accounts to the unfollow queue (after confirm). The queue is a column on the right. You can change the cutoff, select more rows, and add them while it runs. Pause / Resume / Stop, or remove someone from the queue before they are unfollowed. X’s documented limit for `DELETE /2/users/:source_user_id/following/:target_user_id` is **50 requests per 15 minutes per user**. The queue waits a **random 45–110 seconds** between calls (about 8–20 per 15 minutes), including before the first call, and if X returns **429** it waits for `retry-after` / `x-rate-limit-reset` plus extra jitter. A successful row is removed from the CSV table. Other API errors stay on screen and the queue continues.
