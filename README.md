...
6. **Unfollow** or **Unfollow all (selected)** adds accounts to the unfollow queue on the right (after confirm). Press **play** to start; **pause** to hold. The queue stays visible, play/pause are disabled while it is empty, and it pauses itself when it runs out. **Clear** empties the queue without resetting the session unfollow count. Remove someone with the X before they are unfollowed. The session block at the top of the queue counts successful unfollows in this browser tab and shows a **min–max finish time** for the remaining queue (clock range plus duration), using the 45–110s gap per account. If the queue is paused, that range is from pressing play now. X’s documented limit for `DELETE /2/users/:source_user_id/following/:target_user_id` is **50 requests per 15 minutes per user**. The queue waits a **random 45–110 seconds** between calls (about 8–20 per 15 minutes), including before the first call, and if X returns **429** it waits for `retry-after` / `x-rate-limit-reset` plus extra jitter (that extra wait is not in the estimate). A successful row is removed from the CSV table. Other API errors stay on screen and the queue continues.
